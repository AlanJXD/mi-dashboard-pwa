...
const VAPID_KEY =
  "BMA-uNel55_quBhohfPpaj779VEL2lfhYA8CzgkzVtsOIIUuZN4cWsH7T9R0kbp8AQEmhOW7ApBxK1MkhFy8rTo";
...
export const registerForAutoNotifications = async (token: string) => {
  try {
    ...
    if (result.success) {
      console.log(
        "✅ Registrado para notificaciones automáticas cada 2 minutos"
      );
      localStorage.setItem("auto_notifications", "enabled");
      return true;
    } else {
      console.error(
        "❌ Error registrando notificaciones automáticas:",
        result.error
      );
      return false;
    }
